...
This is an open-source civic-commons project (Apache-2.0). An earlier prototype carried different public-facing framing; the Week 1 sanitisation pass (see `docs/grant/02-execution-plan.md` task 1.4) replaced the public-facing residue with `helpmefindthejob.org` placeholders and relocated internal-only working docs to `private/`. The active project direction is civic-commons. Early git commits may still reference legacy strings; see `CONTRIBUTORS-NOTE.md` for the history-preservation rationale.
...
4. **Never commit secrets, real keys, or PII.** Sanitise before pushing. An earlier sanitisation pass cleared the `khalo.org` and tester-name residue from the current working tree; do not reintroduce it. Git history is preserved (not rewritten).
...
7. **Anchor persona panel**: every public-facing example, screenshot, narrative uses one of the seven personas — five most-acute migrant (Aïcha, Yusuf, Olga, Mahmoud, Maria) plus two wider friction-class (Käthe, Tobias). Primary narrative weight stays with the migrant five — the wider two demonstrate the friction-class claim architecturally. See `docs/grant/07-personas.md`.
...
| Housing-agent integration | Option A (mock stub) default; Option B (real, via friend) only if collaborator confirms; self-built (C/D) deferred to Phase 2 |
...
| `docs/mcp-server.md` | Public MCP server documentation |
| `docs/esco-integration.md` | ESCO + EURES integration reference |
| `private/` (gitignored) | Internal-only working docs relocated from `docs/` during Week 1 sanitisation. Not for public reference. |
...
- The Week 1 task 1.4 sanitisation pass replaced `khalo.org` references with `helpmefindthejob.org` placeholders, neutralised tester-name leakage in code and test comments, and relocated internal-only working docs to `private/` (gitignored). Do not reintroduce.
- The historical `cryptography` / `cffi` build issue (Linux without pre-built wheels + no rust + build-essential) is closed. `cryptography` is now explicitly pinned in `requirements.txt` to a range with broad wheel coverage (`>=42.0.0,<50.0.0`), and the `fresh-clone-install` CI workflow at `.github/workflows/fresh-clone-install.yml` verifies on every push that `pip install -r requirements.txt` succeeds on `python:3.11-slim` and `python:3.12-slim` followed by a representative smoke-test slice (encryption-at-rest + AEAD fuzzing + audit log + MCP integration). If you still hit a build failure, your environment is missing pre-built wheel support for cryptography on your specific platform; running inside Docker (`docker compose exec helpmefindthejob python3 -m unittest discover -s tests -v`) is a known-good fallback.
