...
- A `cryptography` / `cffi` build issue may manifest on some environments (initially reported on Linux without pre-built wheels). On the maintainer's macOS host the 925-test suite passes in ~18 s on bare metal; the Week 1 task 2.0 verification pass confirmed no install friction there. Week 3 task 3.1 investigates the failure mode if it reproduces in CI or on a fresh contributor clone. If you hit it, run the suite inside Docker (`docker compose exec directjob-scout python3 -m unittest discover -v`) as a known-good fallback.
