...
def build_letter_prompt(
    *,
    ...
    system = (
        ...
        "8. Unterschrift line\n"
        "9. After the Unterschrift line, emit a blank line then a "
        "clearly-separated block titled '## Quellen (Source citations)' "
        "— this block is NOT part of the letter, it is for the "
        ...
        "claim in the body. This implements the source-class hierarchy "
        "doctrine at docs/grant/14-source-class-hierarchy.md.\n\n"
        "STRICT RULES:\n"
        ...
        "  else English.\n"
        "- Output the letter as plain text. No markdown. No bullet "
        "  lists or enumerated lists inside the Hauptteil — the body "
        "  is flowing prose paragraphs (DIN 5008 Bewerbungsschreiben "
        "  norm).\n"
        "- Job-specificity (NON-NEGOTIABLE): the Hauptteil MUST "
        "  reference at least 2 DISTINCT facts from the <job> tag — "
        "  (1) the role title in the Betreff AND opening, AND (2) at "
        "  least one specific responsibility / required skill / "
        "  company-context detail in the Motivation or Fit paragraph. "
        "  Source restriction is absolute: if the <job> tag does not "
        "  state a fact, do NOT invent it. If <job> details are "
        "  sparse, write generic-but-honest language anchored to what "
        "  IS provided (the role title, the location, the company "
        "  name).\n"
        "- Forbidden filler phrases (case-insensitive — do NOT use "
        "  these tired generic-application markers in the letter "
        "  body): 'Hiermit bewerbe ich mich', 'I am writing to "
        "  express my interest', 'I would like to apply for the "
        "  position', 'In conclusion', 'Overall, this is', 'It is "
