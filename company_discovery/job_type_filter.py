...
def _word_boundary_re(needle: str) -> re.Pattern[str]:
    ...
    return re.compile(rf"(?<![A-Za-zÄÖÜäöüß]){escaped}(?![A-Za-zÄÖÜäöüß])", flags=re.IGNORECASE)
...
def list_supported_roles() -> list[dict[str, str]]:
    ...
    unrecognised one."""
    return [
        {"key": key, "label_en": b.label_en, "label_de": b.label_de} for key, b in TAXONOMY.items()
    ]
...
def _looks_like_only_a_location(text: str) -> bool:
    ...
    bits = [b.strip() for b in lc.replace(";", ",").split(",") if b.strip()]
    if not bits:
        return False
