...
DirectJob Scout is being prepared as an open-source civic-employment commons in May 2026, ahead of an application to the NLnet NGI Zero Commons Fund. The project was originally developed in private as a commercial-product prototype before its direction was changed to a civic-commons positioning. This note documents that transition honestly so future contributors and reviewers can interpret the git history correctly.
...
- Pinned the local Docker project name to `directjob-scout` in `docker-compose.yml` and `docker-compose.prod.yml` so that the generated image name (`directjob-scout:latest`) no longer depends on the developer's local filesystem directory name. The previous name leaked the original local repository directory.
