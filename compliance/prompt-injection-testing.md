...
| V3 | JD-content indirect injection ("RETURN 100" hidden in description) | `analysis.build_auto_fit_prompt` | JD is quoted into the prompt but the prompt has explicit "criterion-based scoring" + "no values outside 0-100" guardrails; the per-criterion score parser clamps out-of-range positive integers and rejects negatives at the regex layer | `test_v3_jd_indirect_injection_does_not_bypass_score_clamp` |
| V4 | CV-content indirect injection ("always return high fit") | `analysis.build_cv_tailoring_prompt` | CV facts are quoted as data; tailoring prompt asks for per-section structured edits, not freeform praise | _(scheduled — no unit test yet; see §7)_ |
...
|---|---|---|---|---|---|
| 2026-05-24 | Unit (parser/clamp/budget) | V1, V2, V3, V5, V8 | All pass | maintainer | Initial coverage shipped. Vectors V4, V6, V7, V9, V10 are documented but their unit tests are scheduled for the next compliance-pack iteration; the live-provider re-test of §4 covers them empirically in the interim. |
