...
| Discovered jobs (system-curated) | Title, employer, posting URL, source, location, ESCO mapping, fit-score | Local DB | At rest only by storage engine; not field-level | Yes — sent in prompt context for fit-scoring and tailoring | Per the user's `retention_days` profile setting (default 90 days; swept by the daily `run_retention_purge` job) |
...
|---|---|---|
| Fit-score (`execute_auto_fit` / `build_auto_fit_prompt`) | Job posting text + relevant CV section (matched by ESCO codes) | Full profile, job-history, email, name |
...
The data-quality controls are tested in the standing 3,348-test suite, including:
...
- **Right to object / withdraw consent**: the user can revoke AI-provider consent at any time via `/api/ai-provider`; subsequent flows fall back to the no-AI path (a deterministic templated skeleton for letter drafting; a BYO-AI handoff prompt for fit-scoring and CV-tailoring).
