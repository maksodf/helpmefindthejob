    ...
    def test_round_trip_and_expiry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                jobs = [
                    AggregatedJob(
                        title="A", company_name="Co", source="x", source_url="https://e/1"
                    )
                ]
        ...
        "data": [
            {
                ...
                "tags": ["python"],
                "url": "https://www.arbeitnow.com/view/abc",
            ...
            },
            {
                ...
                "tags": ["react"],
                "url": "https://www.arbeitnow.com/view/def",
...
_MUSE_BODY = json.dumps(
    {
        "results": [
            {
                "id": 1,
                "name": "Senior Healthcare Policy Consultant",
...
_HN_THREAD_BODY = json.dumps(
    {
        ...
        "children": [
            {
                "id": 1,
                "text": "PPH Consulting | Healthcare Policy Consultant | Berlin, DE\n\nWe are hiring policy consultants for healthcare reform projects in DACH.",
                "created_at": "2026-05-01T00:00:00Z",
            },
            {
                "id": 2,
                "text": "Acme | Senior Backend Engineer | Remote\n\nPython, Postgres, etc.",
...
class BundesagenturProviderTests(unittest.TestCase):
    def test_parses_stellenangebote(self) -> None:
        body = json.dumps(
            {
                "stellenangebote": [
                    ...
                    {
                        "hashId": "def456",
                        "titel": "Healthcare Policy Analyst",
        ...
        self.assertIn("Berlin", out[0].location or "")
        self.assertTrue(
            out[0].source_url.startswith("https://www.arbeitsagentur.de/jobsuche/jobdetail/")
        )
...
class DefaultProvidersTests(unittest.TestCase):
    def test_six_providers_present(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                names,
                ["arbeitnow", "bundesagentur", "hn_hiring", "muse", "remotive", "weworkremotely"],
            )
        finally:
