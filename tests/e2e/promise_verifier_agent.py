...
def verify_P1_no_duplicates() -> Result:
    ...
    try:
        client = _get_or_create_tester(
            f"p1+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        # Run the same search twice
        status1, p1 = client.post(
            "/api/jobs/search",
            {
        ...
        if dup1 or dup2:
            return Result("P1", label, False, f"within-run dups: run1={dup1}, run2={dup2}")
        return Result(
...
def _import_one_job(client: _HttpClient, query: str, location: str) -> dict | None:
    ...
    ss_status, ss_payload = client.post(
        "/api/saved-searches",
        {
    ...
    if dbg:
        print(
            f"   [dbg] discovered count: {len(discovered)}; "
...
def verify_P2_fit_scoring_prompt() -> Result:
    ...
    try:
        ...
        client = _get_or_create_tester(
            f"p2+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        ...
        if isinstance(brief_text, dict):
            brief_text = (
                brief_text.get("prompt") or brief_text.get("output") or brief_text.get("text") or ""
            )
        if not isinstance(brief_text, str) or not brief_text:
            return Result("P2", label, False, f"brief payload has no prompt: {list(payload)}")
        if "UNIQUE_CV_MARKER_P2_X9KT3R" not in brief_text:
            return Result(
                "P2", label, False, f"brief did not include CV marker (len={len(brief_text)})"
            )
...
def verify_P3_tailor_cv_prompt() -> Result:
    ...
    try:
        ...
        client = _get_or_create_tester(
            f"p3+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        _set_cv(client, cv_unique)
        job = _import_one_job(client, "senior backend engineer", "Berlin")
        if not job:
        ...
        out = tailored.get("output") or tailored.get("prompt") or payload.get("output") or ""
...
def verify_P4_skill_gap_atlas() -> Result:
    ...
    try:
        client = _get_or_create_tester(
            f"p4+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        ...
        for query in [
            ...
            if imported >= 3:
                break
        if imported < 2:
            return Result("P4", label, False, f"could only import {imported} jobs from live feeds")
        ...
        if not top:
            ...
            sample = [
                {"title": (j.get("title") or "")[:50], "desc_len": len(j.get("description") or "")}
...
def verify_P5_application_lifecycle() -> Result:
    ...
    try:
        client = _get_or_create_tester(
            f"p5+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        ...
        imported_jobs = (
            boot.get("importedJobs") or boot.get("imported_jobs") or boot.get("imported") or []
        )
        target = next(
            (
                j
                for j in imported_jobs
                if j.get("id") == job.get("id") or j.get("source_url") == job.get("source_url")
            ),
            None,
        )
        if not target:
            return Result(
                "P5", label, False, f"imported job not in bootstrap (n={len(imported_jobs)})"
            )
        st = target.get("application_status") or target.get("applicationStatus")
        # The product stores replied as a timestamp (replied_at). Truthy = replied.
        replied = (
            target.get("replied_at")
...
def verify_P6_watchlist_scan() -> Result:
    ...
    try:
        ...
        if status not in (200, 202):
            return Result("P6", label, False, f"watchlist scan HTTP {status}: {scan}")
...
def verify_P7_cv_encrypted_at_rest() -> Result:
    ...
    try:
        ...
        client = _get_or_create_tester(
            f"p7+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        _set_cv(client, cv_text)
        if not DATA_DIR:
            return Result("P7", label, False, "DATA_DIR env not set — cannot grep DB on disk")
        # Find the SQLite file under data dir. The product uses .sqlite3
        # with WAL — scan all .sqlite* files (includes .sqlite3-wal).
        db_candidates = list(Path(DATA_DIR).rglob("*.sqlite*")) + list(Path(DATA_DIR).rglob("*.db"))
        if not db_candidates:
            return Result("P7", label, False, f"no .db file under {DATA_DIR}")
        ...
        if leaks:
            return Result("P7", label, False, f"plaintext sentinel found in: {leaks}")
        return Result(
            "P7",
            label,
            True,
...
def verify_P8_reply_rate_analytics() -> Result:
    ...
    try:
        client = _get_or_create_tester(
            f"p8+{secrets.token_hex(3)}@example.com", "test-pass-99-xyz!"
        )
        ...
        for query in [
            ...
            if len(ids) >= 3:
                break
        if len(ids) < 3:
            return Result("P8", label, False, f"could only import {len(ids)} jobs from live feeds")
        ...
        for i, jid in enumerate(ids):
            ...
            if status != 200:
                return Result("P8", label, False, f"app update HTTP {status} for {jid}")
        status, boot = client.get("/api/bootstrap")
        if status != 200:
        ...
        summary = (
            boot.get("applicationOutcomes")
            or boot.get("application_outcomes")
            or boot.get("replyRate")
            or {}
        )
        total = (
        ...
        replied = summary.get("replied") or summary.get("repliedCount")
        rate = summary.get("replyRate") or summary.get("rate")
        if not summary:
            return Result(
                "P8", label, False, f"bootstrap has no replyRate payload: {list(boot)[:20]}"
            )
        if (total or 0) != 3 or (replied or 0) != 1:
            return Result(
                "P8", label, False, f"totals off: applied={total}, replied={replied}, rate={rate}"
            )
