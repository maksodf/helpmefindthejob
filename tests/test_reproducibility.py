...
class TrackedEntriesContract(unittest.TestCase):
    def test_modes_are_git_canonical(self):
        for mode, _path in tracked_entries(REPO_ROOT):
            self.assertIn(mode, ("100644", "100755", "120000", "160000"))

    def test_verify_script_is_executable_in_index(self):
        modes = {path: mode for mode, path in tracked_entries(REPO_ROOT)}
        self.assertEqual(
            modes.get("scripts/verify_reproducibility.sh"),
            "100755",
            "verify_reproducibility.sh must be tracked executable",
        )
