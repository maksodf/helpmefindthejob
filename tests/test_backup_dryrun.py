...
def _run(env_extra: dict[str, str]) -> tuple[int, str, str]:
    env = {**os.environ, **env_extra, "DIRECTJOB_BACKUP_DRY_RUN": "1"}
    res = subprocess.run(
        ["sh", str(SCRIPT)],
        cwd=ROOT,
        env=env,
...
class BackupDryRunTests(unittest.TestCase):
    ...
    def test_local_dryrun_succeeds(self) -> None:
        code, stdout, stderr = _run({"DIRECTJOB_BACKUP_BACKEND": "local"})
        self.assertEqual(code, 0)
        self.assertIn("DRY RUN", stdout)
        self.assertIn("would write tarball", stdout)

    def test_unknown_backend_exits_2(self) -> None:
        code, _, stderr = _run({"DIRECTJOB_BACKUP_BACKEND": "mystery"})
        self.assertEqual(code, 2)
        self.assertIn("unknown DIRECTJOB_BACKUP_BACKEND", stderr)
    ...
    def test_offhost_without_cli_or_remote_exits_2(self) -> None:
        ...
        code, _, stderr = _run({"DIRECTJOB_BACKUP_BACKEND": "rclone"})
        self.assertEqual(code, 2)
        self.assertTrue(
            "rclone not installed" in stderr or "DIRECTJOB_BACKUP_REMOTE missing" in stderr,
            stderr,
        )
