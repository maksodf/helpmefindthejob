...
| 77 | **Streaming refactor for chat-message endpoint** — combined slice covering: (a) Server-Sent Events on `/api/chat/message` replacing single-shot JSON; (b) Concurrent aggregator fan-out via `asyncio.gather(...)` emitting per-provider events (today `aggregators.py:874` runs `for provider in self.providers:` sequentially); (c) Token-streamed AI responses via `stream=True` (Ollama, Anthropic, OpenAI all support it) plumbed through `_dispatch_provider` in `company_discovery/analysis.py`; (d) Frontend in-place bubble mutation in `static/app.js` consuming the event stream and appending tokens/events to a `data-pending="true"` placeholder bubble. Ships full LIVE progress narration (per-provider search progress + token-by-token AI output) AND closes Gate 6.7 (partial results streaming). Loop 14 read-through confirmed zero streaming infrastructure today; Loop 14.1 ships static phase-aware typing labels (closes Gate 6.6) but cannot close 6.7 without this slice. Effort: ~2-3 d. Schedule: 2026 Q3-Q4. Trigger: post-grant kickoff. **Coupling**: don't ship in pieces — concurrent fan-out without streaming = still silent end-to-end (worse UX than today). All four sub-pieces ship together. | 2-3 d | 2026 Q3-Q4 |
...
- **Items remaining**: 64.
- **Targets**: 2026 Q3 = 31 items; 2026 Q4 = 19 items; 2027 Q1 = 12 items; trigger-driven = 2.
