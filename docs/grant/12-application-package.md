...
- **One-line summary**: An open-source EU-wide civic employment commons that captures specialist HR and bureaucratic-navigation knowledge into modular, MCP-composable tools, with first reference deployment for migrant jobseekers in Germany.
...
> Across the European Union, structural labor shortages coexist with hundreds of thousands of qualified migrants who cannot get hired — blocked not by capability but by friction: language barriers, opaque foreign-credential recognition, bureaucratic fragmentation, platform monopolies that profile and lock in personal data, and AI tooling that requires commercial subscriptions the unemployed cannot afford.
>
...
The EU faces a structural labor-market paradox. As of 2024, the German Federal Employment Agency identifies 163 of approximately 1,200 occupations in shortage, with ~46,000 unfilled positions in healthcare alone. Comparable shortages exist across the EU. In parallel, hundreds of thousands of qualified migrants — including all EU citizens with limited host-language fluency, Ukrainian refugees under the Temporary Protection Directive (whose job-finding rate has dropped since 2022 specifically due to language barriers, per OECD 2025), Triple Win programme participants (nurses recruited from Tunisia, Philippines, Vietnam, Bosnia), subsidiary-protection holders from Syria seeking Ausbildung placements, and EU-mobile Romanian and Bulgarian workers — cannot get hired.

The blocker is rarely capability. The blocker is friction. Bureaucratic systems run in a language the migrant does not speak. Foreign credentials require Anerkennung through processes that vary by Bundesland and profession. Application formats and CV conventions differ from those the migrant has learned. The job market is fragmented across platforms (LinkedIn, Indeed, Bundesagentur's JOBBÖRSE, sector-specific portals) — each requires its own account, profile, and learning curve. Each platform extracts personal data and locks the user in.
...
Meanwhile, the institutions that serve migrant jobseekers — Migrationsberatungsstellen (~700 across Germany), IQ-Netzwerk's 16 regional networks, the 104 autonomous Optionskommunen Jobcenter, university career services for international graduates — operate under acute capacity constraints. Advisor time is the bottleneck. Translation services are expensive. AI tooling that could help is gated behind commercial pricing or compliance risk most institutions cannot absorb.

The opportunity is to build the missing layer: an open-source, multilingual, privacy-preserving civic employment agent that is **both** usable directly by migrants **and** deployable by the institutions that serve them — reducing institutional operational cost while improving end-user outcomes, simultaneously.
