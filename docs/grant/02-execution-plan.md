...
### 1.5 GitHub repo housekeeping (1 h) — maintainer-clicks, prepared text
...
- Production-deployment alignment closed — mitigated by Decision 17, no public deployment exists *(the §3.4 parallel-public-instance posture decided 2026-05-18 is a not-yet-stood-up deliverable; this Week 1 statement remains accurate for the **private** single-tester deployment).*
...
- [x] `compliance/human-oversight-guide.md` + minimum-viable human-oversight UI (Article 14): admin endpoint `/api/admin/oversight/queue` gated by `HELPMEFINDTHEJOB_HUMAN_OVERSIGHT_MODE`; four oversight modes (passive monitoring / advisor-review queue / per-action gates / kill-switch); appointment record with TBD placeholders
...
- [x] Add coverage upload to Codecov — CI workflow `.github/workflows/quality.yml` (`coverage` job). Uses `coverage run -m unittest discover -s tests` then `codecov/codecov-action@v5`. Branch coverage enabled in `pyproject.toml` `[tool.coverage.run]`. README badge added.
...
- [~] Add multi-OS matrix — **partial**. `.github/workflows/test.yml` matrices over Python 3.11 + 3.12 on `ubuntu-latest`. Cross-platform (macOS, Windows) coverage was attempted and reverted in the same task because the existing test suite has Linux-isms requiring systematic remediation: (a) many tests open files without explicit `encoding="utf-8"`, so Windows runners fall back to cp1252 and the i18n / encryption / rate-limit tests fail in cascade; (b) subprocess-based HTTP smoke tests (`test_app_smoke`, `test_http_admin_extras`, `test_http_phase2`) time out in `setUpClass` on macOS runners with `RuntimeError: server did not start` (slower TCP-bind cycle than Ubuntu). Both deferred to Phase 2 cleanup (tracked in `docs/grant/03-post-grant.md`). The workflow comments document the deferral so future contributors find context.
...
- [x] **SHA-pin all GitHub Actions** (mechanical hardening pass, 2026-05-19): `fresh-clone-install.yml`, `mcp-integration.yml`, `quality.yml`, `scorecard.yml` — every `uses:` reference now uses full 40-char commit SHA with inline patch-version comment (16 references across 4 files). Closes the OpenSSF Scorecard "pinned-dependencies" gap for these workflows; `docs-publish.yml` was already SHA-pinned in 9b3cac7. **`test.yml` SHA-pinned in follow-on slice (2026-05-19)**: 2 references (checkout, setup-python) pinned to the same SHAs the prior sweep resolved. `grep -nE "uses:.*@v[0-9]" .github/workflows/*.yml` now returns zero lines — every workflow file in the repo (`fresh-clone-install.yml`, `mcp-integration.yml`, `quality.yml`, `scorecard.yml`, `test.yml`, `docs-publish.yml`) is fully SHA-pinned, closing the OpenSSF Scorecard "pinned-dependencies" sweep across all six workflow files.
...
- [x] Run an automated accessibility audit (axe-core or WAVE) against the public demo *(landed 2026-05-19: axe-core CLI 4.11.4 against 13 public surfaces — 8 web-app HTML pages on local app.py + 5 mkdocs-material docs pages on local `mkdocs serve` (GH Pages still off, audit-equivalent build per audit-time). Raw axe JSON for every surface committed under `audit-results/`. First-pass: 8 violations / 17 nodes across 4 of 13 surfaces.)*
- [x] **Authenticated-surface audit** *(follow-on slice landed 2026-05-19: closes the §3.6 ce48ba5 adversarial-self-review #1 gap. New `scripts/accessibility-audit-auth-surfaces.py` runner using Playwright 1.60.0 + axe-playwright-python 0.1.7 (axe-core 4.10) seeds Aïcha persona, logs in via the sign-in form, walks through 8 authenticated UI states (sign-in landing regression check + post-login + jobs + assistant + companies + cv-builder + settings + dashboard), captures screenshot + axe JSON + violation summary per state. First-pass: 14 violations (0 critical, 8 serious, 6 moderate) across 7 of 8 states. After-fix: 0 violations on every state. Same honesty discipline as the §3.6 first pass — no tolerance manipulation.)*
- [x] **Polish trio — light-mode auth-surface + compliance markdown + dynamic-state coverage** *(landed 2026-05-19: closes the three known-gaps the auth-surface slice left open. Sub-slice A: runner accepts `--color-scheme {dark,light,both}` and emulates `prefers-color-scheme` + flips the in-app `applyTheme()` after login; 8 light-mode auth states audited; 7 violations pre-fix → 0 post-fix. Sub-slice B: `compliance/transparency-notice.md` + `compliance/deployer-operating-manual.md` brought into the mkdocs nav via `docs/hooks/compliance_mirror.py` build hook (canonical source stays at repo root, mirror is regenerated each build, in `.gitignore`); axe-core CLI 4.11 audit returned 0 violations on both pages after 6 cross-tree-link warnings were closed by rewriting source links to absolute GitHub URLs. Sub-slice C: runner extended with 4 dynamic-state audits (cmdk dialog open via Ctrl+K, Suggestions `<details>` disclosure expanded, settings rendered, login error after bad-password submit with `context.clear_cookies()` to drop the auth session); 1 violation pre-fix (cmdk results listbox lacked accessible name) → 0 post-fix. Combined across all three passes: 30 violation instances closed across all 33 audited captures, axe defaults preserved.)*
- [x] Fix the highest-impact issues *(landed 2026-05-19: (1) `static/index.html` auth gate `<section>` → `<main>` with `aria-labelledby` — fixed `landmark-one-main` + `region` on the app landing page (2 violations / 5 nodes → 0); (2) `docs/stylesheets/accessibility.css` underline rule scoped to `.md-content` body links — fixed `link-in-text-block` on the docs landing (3 nodes → 0); (3) `docs/javascripts/accessibility.js` runtime hook setting `aria-label="Site search"` on mkdocs-material's `.md-search[role="dialog"]` — fixed `aria-dialog-name` across all 5 docs pages (5 violations → 0). Re-audit verified zero violations on every audited surface. Tolerance NOT manipulated.)*
- [x] Document the current state in `ACCESSIBILITY.md` honestly: WCAG 2.2 Level AA as target, known gaps, remediation plan *(landed 2026-05-19: `ACCESSIBILITY.md` at repo root with stated target, honesty note about scope (authenticated app surfaces + compliance markdown + manual / keyboard / screen-reader review deferred), audit methodology + tool version + URLs, current-state table (pre-fix vs post-fix per surface), per-fix file:line + before/after, 6-row known-gaps remediation table with timeline, NLnet HAN University audit reference, reproduction commands, audit-evidence index. Cross-linked from README.md + mkdocs.yml nav + STANDARDS.md already cites it.)*
- [ ] Optionally request HAN University accessibility support if Commons Conservancy admission has come through *(deferred: conditional on Commons Conservancy admission — surfaced in `ACCESSIBILITY.md` "NLnet support services" section as the pathway when admission lands.)*
...
- [x] **i18n hardening** *(follow-on slice landed 2026-05-19: closes the three 6e403c9 adversarial-review findings. (i) Parity test generalised to walk every `static/i18n/*.json` — no more EN+DE hard-code; adding Arabic/Ukrainian/Turkish/Romanian gates the new bundle from PR-day-one with no test edit. (ii) `PRESERVED_TERMS` constant added with the 29 bureaucratic terms from docs/translating.md; new test machine-enforces the German-conventions preservation rule wherever en.json uses any listed term. (iii) New `test_html_inline_fallback_matches_en_json` catches drift between `<element data-i18n="key">text</element>` HTML fallback and `en.json[key]` — discovered + fixed 8 pre-existing drift instances (`dashboard.metric.companies`, `findJobs.limit`, `companies.suggestions.refresh`, `settings.tab.security`, `settings.bookmarklet.dragMe`, `settings.ai.heading`, `settings.ai.lead`, `view.searchResults.subtitle`) where the HTML had wandered from en.json. All 5 contracts demonstrated via manual-break-and-revert. 1018 → 1020 tests; no production code paths touched outside the 8 HTML fallback alignments.)*
...
- [x] Add `flake.nix` at repo root producing a reproducible build *(landed 2026-05-19: `flake.nix` + `flake.lock` at repo root. Inputs pinned to `nixpkgs` `nixos-25.05` (commit `ac62194`) + `flake-utils` (commit `11707dc`) + `flake-utils/systems` (commit `da67096`). Outputs: `devShells.<system>.default` provides Python 3.12 + OS-level toolchain (git, curl, gnumake, pkg-config); shellHook bootstraps `.venv/` from `requirements.txt` + `requirements-dev.txt` on first entry with a `.deps_installed` sentinel for cache-on-subsequent-entry. `apps.<system>.default` (`nix run`) wraps the same bootstrap + launches `python3 app.py`. `packages.<system>.default` intentionally not provided — hybrid Nix-native interpreter + pip-managed deps is the Phase 1 choice; documented in `flake.nix` header comment + `docs/deployment-recipe.md` §12. Outputs use `flake-utils.lib.eachDefaultSystem` for x86_64-linux / aarch64-linux / x86_64-darwin / aarch64-darwin coverage without per-system copy-paste.)*
- [x] Document usage in `docs/deployment-recipe.md` and CONTRIBUTING.md *(landed 2026-05-19: `docs/deployment-recipe.md` §12 "Reproducible builds (Nix flake)" with Why-this-exists / Requirements / Commands / Hybrid-rationale / Pinned-input / Verified-on subsections; `CONTRIBUTING.md` "Nix workflow (optional, reproducible)" subsection under Development Setup positioning Nix as alternative path that doesn't displace pip. `STANDARDS.md` Nix row flipped from Planned → Shipped with file links. `mkdocs.yml` already exposes deployment-recipe + CONTRIBUTING via nav; no nav change needed.)*
- [x] Verify the flake builds on a clean machine *(landed 2026-05-19: Determinate Nix 3.20.0 / Nix 2.34.6 on aarch64-darwin freshly installed. `nix flake check` green; first-build `nix develop` 51 s wall-clock including venv bootstrap; subsequent entries instant via the `.deps_installed` sentinel. `nix develop --command python3 -m unittest discover -s tests` runs 1029 tests / 12 skipped under Python 3.12 (vs bare-host 1020 / 4 skipped under miniconda Python 3.9 — Python 3.12 collects 9 more test files that contain 3.10+-only syntax `match`/walrus/PEP-604-union which 3.9 silently doesn't collect). `nix develop --command ruff check` + `nix develop --command codespell` both clean. Cross-system verification on x86_64-linux + aarch64-linux + x86_64-darwin is Phase 2 (the existing `fresh-clone-install.yml` already covers Linux pip-path reproducibility via the matrix).*
- [ ] Optionally request NixOS Foundation packaging support if Commons Conservancy admission has come through *(deferred: conditional on Commons Conservancy admission gating, same as the NLnet HAN University accessibility-audit pathway; surfaced in `flake.nix` header comment as the trigger for the future pure-Nix `buildPythonApplication` slice.)*
...
- [x] Confirm the send-gate criterion is satisfied *(landed 2026-05-19: full send-gate audit landed at the time. The dated `outreach-readiness-2026-05-19.md` artefact was removed in the 2026-05-23 docs cleanup; the active outreach state now lives in `11-institutional-outreach.md`. 11 of 12 criteria show ✅; the docs-site live-deploy line is 🟡 amber pending the working-branch-to-main merge that triggers the Pages deploy workflow. Recommendation: send AFTER the merge so the docs URL referenced in the email bodies resolves on first click.)*
