...
- [x] **SHA-pin all GitHub Actions** (mechanical hardening pass, 2026-05-19): `fresh-clone-install.yml`, `mcp-integration.yml`, `quality.yml`, `scorecard.yml` — every `uses:` reference now uses full 40-char commit SHA with inline patch-version comment (16 references across 4 files). Closes the OpenSSF Scorecard "pinned-dependencies" gap for these workflows; `docs-publish.yml` was already SHA-pinned in 9b3cac7. **`test.yml` still tag-pinned** — surfaced in the slice's adversarial self-review as a follow-up; the slice spec named 4 files explicitly and I honored that scope, but a future small slice should sweep `test.yml` too.
